plsBiplot <- function(oplsda, y, 
                      saveFlag = FALSE, fileStr = "",
                      color1 = "red", color2 = "blue", 
                      shape1 = 21, shape2 = 21, fontsize = 4, alpha_loading = 1,
  ...
  dfScores <- data.frame(LV1 = scoresLV1, LV2 = scoresTmp[,1], class = factor(y))
  ...
  pltScores <- ggplot(dfScores, aes(LV1, LV2, color = class, shape = class)) + 
    ...
    geom_vline(xintercept = 0, size = 0.3) + geom_hline(yintercept = 0, size = 0.3)
  if (saveFlag) {
  ...
  dfLoadings <- data.frame(LV1 = loadingsLV1, LV2 = loadingsTmp[,1])
  ...
  pltLoadings <- ggplot(dfLoadings, aes(LV1, LV2, label = rownames(dfLoadings))) + 
    ...
    geom_segment(aes(x = x1, y = y1, xend = x2, yend = y2), alpha = alpha_loading,
                 data = arrow.df, 
                 color = feature_annot$useColor[match(rownames(dfLoadings),
                                                      rownames(feature_annot))]) +
    geom_text_repel(size = fontsize, aes(label = feature_annot$label[match(rownames(dfLoadings), 
                                                                           rownames(feature_annot))]), 
                                         color = feature_annot$useColor[match(rownames(dfLoadings),
                                                                              rownames(feature_annot))]) +
    theme(panel.background = element_blank(),
