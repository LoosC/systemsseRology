...
#     class = c(class1 = "#1f618d", class2 = "#5dade2", class3 = "red")
#     )
```
...
#y <- rep(c(0,1), c(10, 20))
#yName <- factor(rep(c("class1", "class2", "class3"), c(10, 15, 5), levels = c("class1", "class2", "class3")))
...
pre_symbol <- try(oplsda <- opls(X_sel, yName,  permI = 0, crossValI = 5))
isError <- is(pre_symbol, 'try-error')
if (isError) {
  oplsda <- opls(X_sel, yName, predI = 1,  permI = 0, crossValI = 5)
}
```
...
```{r Visualization} 
feature_annot$useColor <- myColors$antigen[feature_annot$antigen]
plsBiplot(oplsda, yName, colors_y = myColors$class, alpha_loading = 0.5,
          feature_annot = feature_annot)

# to test color
library(RColorBrewer)
plsBiplot(oplsda, yName, alpha_loading = 0.5,
          feature_annot = feature_annot)

# just to test regression plot
plsBiplot(oplsda, 1:length(yName), alpha_loading = 0.5,
          feature_annot = feature_annot, type = "regression")
