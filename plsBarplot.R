plsBarplot <- function(oplsda, y, X_sel,
                       saveFlag = FALSE, fileStr = "",
                       color1 = "red", color2 = "blue", feature_annot = data.frame()){

  if (dim(feature_annot)[1] == 0) {
    feature_annot <- data.frame(useColor = rep("black", dim(X_sel)[2]), label = colnames(X_sel))
  ...
  # loadingsLV1 are still used from the orthogonal PLS-DA (oplsda) 
  # above because they discriminante between classes for </> 0
  ...
  # plot loadings sorted according to the VIP score and color coding it according to enrichent in classes
  #pltBar <- ggplot(data = df , aes(x = reorder(features, vipScores), y = loadingsLV1, fill = mark))  +
