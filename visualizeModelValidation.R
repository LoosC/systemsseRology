visualizeModelValidation <- function(res, type, saveFlag = FALSE, strSave = "", modelColor = "gray"){
  ...
  if (type == "classification") {
    value <- c(as.vector(res$acc),as.vector(res$accPerm1),as.vector(res$accPerm2))
    dfBox <- data.frame(value = value,
                        model = rep(c("model","random features","permuted labels"),
                                    c(length(as.vector(res$acc)), length(as.vector(res$accPerm1)), length(as.vector(res$accPerm2)))))
    dfBox$model = factor(dfBox$model, levels = c("model","random features","permuted labels"))
    
    myComparisons <- list( c("model", "permuted labels"), c("model", "random features"))
    ...
    dev.off()
    print(pltVio)
    pvals <- data.frame(acc1 = vector(length = length(res$acc)),
                        acc2 = vector(length = length(res$acc)))
    ...
    value <- c(as.vector(res$corr),as.vector(res$corrPerm1),as.vector(res$corrPerm2))
    dfBox <- data.frame(value = value,
                        model = rep(c("model","random features","permuted labels"),
                                    c(length(as.vector(res$corr)), length(as.vector(res$corrPerm1)), 
