modelValidation <- function(X, y, nFolds = 5, nReps = 10, nPerms = 100, nTop = 5, 
                            featureMethod = "lasso", method = "pls", type = "classification",
  ...
  for (iRep in 1:nReps) {
    ...
    for (iFold in 1:nFolds) {
      ...
      selFeatures <- featureSelection(XTrain, as.vector(yTrain), method = featureMethod, type = type, 
                                        chooseS = chooseS, nTop = nTop,
      ...
      if (method == "randomForest") {
        ...
        pre_symbol <- try(trainedModel <- opls(XTrainSel, yTrain, orthoI = NA, permI = 0,
                                               crossValI = 5, info.txtC = "none", fig.pdfC = "none"))
        isError <- is(pre_symbol, 'try-error')
        if (isError) {
          trainedModel <- opls(XTrainSel, yTrain, predI = 1, orthoI = 1, permI = 0,
                               crossValI = 5, info.txtC = "none", fig.pdfC = "none")
        }
      }
      ...
      if (nPerms > 0 & nFolds > 1) {
        for (iPerm in 1:nPerms) { 
          ...
          if (method == "randomForest") {
            ...
            pre_symbol <- try(trainedModel <- opls(XTrainSelPerm1, yTrain, orthoI = NA, permI = 0,
                                            crossValI = 5, info.txtC = "none", fig.pdfC = "none"))
            isError <- is(pre_symbol, 'try-error')
            if (isError) {
              trainedModel <- opls(XTrainSelPerm1, yTrain, predI = 1, orthoI = 1, permI = 0,
                            crossValI = 5, info.txtC = "none", fig.pdfC = "none")
            }
          }
          ...
          # null model 2:
          # feature selection and model training is done with permuted data 
          yPerm <- y[sample(1:length(y), size = length(y), replace = FALSE)]
          yTrainPerm <- yPerm[-folds[[iFold]]]
          selFeaturesPerm <- featureSelection(XTrain, as.vector(yTrainPerm), method = featureMethod, type = type, 
                                                chooseS = chooseS, nTop = nTop,
                                                nFeatRep = nFeatRep, nLassoFolds = nLassoFolds, 
                                                thresh = thresh, alpha = alpha)
          ...
          if (method == "randomForest") {
            ...
            pre_symbol <- try(trainedModel <- opls(XTrainSelPerm2, yTrainPerm, 
                                            orthoI = NA, permI = 0,
                                            crossValI = 5, info.txtC = "none",
                                            fig.pdfC = "none"))
            isError <- is(pre_symbol, 'try-error')
            if (isError) {
              trainedModel <- opls(XTrainSelPerm2, yTrainPerm, predI = 1, orthoI = 1, permI = 0,
                            crossValI = 5, info.txtC = "none", fig.pdfC = "none")
            }
          }
